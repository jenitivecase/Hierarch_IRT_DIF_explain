...
Rprof("fullprofile_single.txt", memory.profiling = TRUE)
...
precomp_model <- stan_model(stanc_ret = precomp, save_dso = TRUE, auto_write = TRUE)
...
for(i in 1:nreps){
  ...
  analysis <- sampling(precomp_model, data = b.dat_long,
                       iter = 12000, warmup = 5000, chains = 1, cores = 1,
                       verbose = FALSE)
...
Rprof(NULL)
summaryRprof(paste0(getwd(), "/../fullprofile_single.txt"), memory = "both")
